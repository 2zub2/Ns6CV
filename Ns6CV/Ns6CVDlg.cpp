...
BEGIN_MESSAGE_MAP(CNs6CVDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON_LOAD_PROPERTY, &CNs6CVDlg::OnBnClickedButtonLoadProperty)
END_MESSAGE_MAP()
...
{
	CFileDialog dlg(TRUE);
	dlg.m_ofn.lpstrFilter = _T("Config file (*.cfg)\0*.cfg\0All Files (*.*)\0*.*\0\0");
...
void CNs6CVDlg::OnBnClickedButtonLoadProperty()
{
	CT2CA pszConvertedAnsiString(edtOptionFileName);
	std::string strStd(pszConvertedAnsiString);
	workImage.readSettings(strStd);
